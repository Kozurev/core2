...
class Admin_Form extends Admin_Form_Model 
{
	...
	public function getListScheduleTypes($aParams)
    {
        $main = new stdClass();
        $main->title = "Основной график";
        $main->id = 1;
        $this->addEntity($main, "item");

        $current = new stdClass();
        $current->title = "Актуальный график";
        $current->id = 2;
        $this->addEntity($current, "item");

        $modelId = Core_Array::getValue( $aParams, "model_id", 0 );
        if( $modelId != 0 )
        {
            $Lesson = Core::factory( "Schedule_Lesson", $modelId );
            $this->value = $Lesson->lessonType();
        }
    }
    ...
    public function getListPaymentType($aParams)
    {
        ...
        $aoTypes = Core::factory("Payment_Type")->findALl();
